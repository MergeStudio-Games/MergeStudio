...
param(
    [string]$Editor = "",
    [string]$Output = "Build/MixoKitchen.apk",
    [switch]$DisableBurst,
...
$log = Join-Path $project 'Logs/Android-APK.log'
New-Item -ItemType Directory -Force -Path (Split-Path $log) | Out-Null
$arguments = @(
    '-batchmode', '-nographics', '-quit', '-projectPath', ('"' + $project + '"'),
    '-executeMethod', 'MergeStudio.Editor.CiBuild.BuildAndroidApk',
    '-logFile', ('"' + $log + '"')
)
...
if ($LASTEXITCODE -ne 0) { throw 'Android APK manifest generation failed.' }
Write-Output "Android APK: $outputPath ($((Get-Item $outputPath).Length) bytes)"
